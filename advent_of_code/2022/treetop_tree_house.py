...
right or bottom since other trees of height 5 are in the way.) The
top-middle 5 is visible from the top and right. The top-right 1 is not
visible from any direction; for it to be visible, there would need to only
be trees of height 0 between it and an edge. The left-middle 5 is visible,
but only from the right. The center 3 is not visible from any direction; for
it to be visible, there would need to be only trees of at most height 2
between it and an edge. The right-middle 3 is visible from the right. In the
bottom row, the middle 5 is visible, but the 3 and 4 are not. With 16 trees
visible on the edge and another 5 visible in the interior, a total of 21
...
def main():
    ...
    """
    with open("treetop_tree_house.txt", encoding="utf-8") as file:
            ...
            for j, cell in enumerate(row):
                if i == 0 or j == 0 or i == n - 1 or j == m - 1:
                    ...
                    for y in range(i - 1, -1, -1):
                        dist += 1
                        max_tree = max(field[y][j], max_tree)
                        if max_tree >= cell:
                            score *= dist
                            break
                    if max_tree < cell:
