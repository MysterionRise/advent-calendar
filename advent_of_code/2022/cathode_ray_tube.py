...
def signal_strength(instructions):
    ...
    cycles = 0
    signal = 0
    # Keep track of whether the signal strength has already been updated for each multiple of 20 cycle.
    signal_updated_for_multiple_of_20_cycles = set()
    for idx, instruction in enumerate(instructions):
        # Execute the instruction on the CPU.
        if instruction[0] == "addx":
            cycles += 1
            # Calculate the closest cycle count that is a multiple of 20.
            closest_multiple_of_20 = cycles - (cycles % 20)
            # If the current cycle is the 20th, 21st, 60th, 61st, 100th, 101st, 140th, 141st, 180th, 181st, or 220th, 221st,
            ...
            if (
                ...
                signal_updated_for_multiple_of_20_cycles.add(closest_multiple_of_20)
...
def output_screen(screen):
    # Iterate over the rows of the screen.
    for row in screen:
        # Print the row as a string of "#" and "." characters.
...
def main():
    ...
    """
    with open("cathode_ray_tube.txt", encoding="utf-8") as file:
        lines = file.readlines()
        lines = [line.strip() for line in lines]

        signal = signal_strength(create_program(lines))
        print(signal)  # -20
