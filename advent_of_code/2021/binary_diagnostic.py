...
next bit to the right. The bit criteria depends on which type of rating
...
oxygen generator rating value using the same example diagnostic report from
above:

Start with all 12 numbers and consider only the first bit of each number.
There are more 1 bits (7) than 0 bits (5), so keep only the 7 numbers with a
1 in the first position: 11110, 10110, 10111, 10101, 11100, 10000,
and 11001. Then, consider the second bit of the 7 remaining numbers: there
are more 0 bits (4) than 1 bits (3), so keep only the 4 numbers with a 0 in
the second position: 10110, 10111, 10101, and 10000. In the third position,
three of the four numbers have a 1, so keep those three: 10110, 10111,
and 10101. In the fourth position, two of the three numbers have a 1,
so keep those two: 10110 and 10111. In the fifth position, there are an
equal number of 0 bits and 1 bits (one each). So, to find the oxygen
generator rating, keep the number with a 1 in that position: 10111. As there
is only one number left, stop; the oxygen generator rating is 10111,
...
def get_data(candidate_lines, is_oxygen_generator=True):
    """
    Get the data from the candidate lines.
    :param candidate_lines: input lines
    :param is_oxygen_generator: if True, get the oxygen generator rating,
    :return:
    """
    not_finished = True
    idx = 0
    while idx <= len(candidate_lines[0]) - 1 and not_finished:
        zero_lines = []
        one_lines = []
        ...
        if is_oxygen_generator:
            if len(one_lines) >= len(zero_lines):
                candidate_lines = one_lines
            else:
                candidate_lines = zero_lines
        else:
            if len(one_lines) < len(zero_lines):
                candidate_lines = one_lines
            else:
    ...
    return int(candidate_lines[0], 2)
...
def main():
    ...
    """
    with open("binary_diagnostic.txt", encoding="utf-8") as file:
        lines = file.readlines()
        oxygen_rating = get_data(lines)
        scrubber_rating = get_data(lines, is_oxygen_generator=False)
