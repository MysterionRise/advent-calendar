...
it doesn't look like they'll be aligned before the whale catches you! Maybe
you can help?
...
def main():
    ...
    """
    with open("treachery_of_whales.txt", encoding="utf-8") as file:
        positions = [int(x) for x in file.read().split(",")]
        best_sum = int(1e9)
        for pos in range(10000):
            result_sum = 0
            for _, val in enumerate(positions):
                result_sum += int(((1 + abs(val - pos)) * abs(val - pos)) / 2)
