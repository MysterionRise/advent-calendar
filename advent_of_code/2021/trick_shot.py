...
Due to gravity, the probe's y velocity decreases by 1. For the probe to
successfully make it into the trench, the probe must be on some trajectory
that causes it to be within a target area after any step. The submarine
...
def simulate_shot(x_coords, y_coords, x_speed, y_speed) -> tuple[bool, int]:
    ...
    and highest y_pos position reached
    """
    x_pos = 0
    y_pos = 0
    idx = 0
...
def main():
    ...
    """
    with open("trick_shot.txt", encoding="utf-8") as file:
        line = file.readline().strip()
        x_coords, y_coords = line.split(",")
        x_coords = [int(x) for x in x_coords[2:].split("..")]
        y_coords = [int(y) for y in y_coords[2:].split("..")]
        x_coords.sort()
        y_coords.sort()
