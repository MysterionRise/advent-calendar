...
it's completely covered by two large herds of sea cucumbers, and there isn't
...
they're predictable critters: they move in perfectly straight lines,
only moving forward when there's space to do so. They're actually quite
polite!"
...
submarine helpfully generates a map of the situation (your puzzle input).
For example:

v...>>.vv> .vv>>.vv.. >>.>v>...v >>v>>.>.v. v>v.vv.v.. >.>>..v... .vv..>.>v.
v.v..>>v.v ....v..v.> Every step, the sea cucumbers in the east-facing herd
attempt to move forward one location, then the sea cucumbers in the
south-facing herd attempt to move forward one location. When a herd moves
forward, every sea cucumber in the herd first simultaneously considers
whether there is a sea cucumber in the adjacent location it's facing (even
another sea cucumber facing the same direction), and then every sea cucumber
...
Sleigh keys detected! According to the console, the keys are directly under
the submarine. You landed right on them! Using a robotic arm on the
submarine, you move the sleigh keys into the airlock.
...
def move_cucumbers_east(field):
    """
    Moves sea cucumbers to the east, if there is an empty space to the right
    :param field:
    :return:
    """
...
def move_cucumbers_south(field):
    """
    Moves sea cucumbers to the south, if there is an empty space below
    :param field:
    :return:
    """
    new_field = field.copy()
...
def main():
    ...
    Main function
    """
    with open("sea_cucumber.txt", encoding="utf-8") as file:
        lines = file.readlines()
    field = [list(line.strip()) for line in lines]
    for i in range(MAX_STEPS):
        is_moved_east, field = move_cucumbers_east(field)
        is_moved_south, field = move_cucumbers_south(field)
        if is_moved_east is False and is_moved_south is False:
            print(i + 1)
            break
