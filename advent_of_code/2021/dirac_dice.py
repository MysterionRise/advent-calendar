...
order of increasing value, wrapping back around to 1 after 10). So,
if a player is on space 7 and they roll 2, 2, and 1, they would move forward
...
def dynamic_programming(position, score, turn, init):
    ...
    """
    if turn == 0:
        return 1 if (score == 0 and position == init) else 0

    if score <= 0:
        return 0

    res = 0
    for possible_score, freq in frequency.items():
        if score - position >= 21:
            continue
        res += (
            dynamic_programming(
                move_player_backward(position, possible_score),
                score - position,
                turn - 1,
                init,
            )
...
def count_wins(pos1, pos2, is_player1):
    """
    calculate number of wins given pos1 and pos2
    """
    res = 0
...
def main():
    ...
    """
    with open("dirac_dice.txt", encoding="utf-8") as file:
        lines = file.readlines()
        player1_start = int(lines[0].strip().split(":")[-1])
        player2_start = int(lines[1].strip().split(":")[-1])
        player1_score = count_wins(player1_start, player2_start, True)
        player2_score = count_wins(player2_start, player1_start, False)
        print(max(player1_score, player2_score))
