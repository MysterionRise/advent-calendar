...
Elves tripped and accidentally sent the sleigh keys flying into the ocean!
...
199  A 200  A B 208  A B C 210    B C D 200  E   C D 207  E F   D 240  E F G
269    F G H 260      G H 263        H Start by comparing the first and
second three-measurement windows. The measurements in the first window are
marked A (199, 200, 208); their sum is 199 + 200 + 208 = 607. The second
window is marked B (200, 208, 210); its sum is 618. The sum of measurements
in the second window is larger than the sum of the first, so this first
...
def main():
    ...
    """
    res = 0
    with open("sonar_sweep.txt", encoding="utf-8") as file:
        lines = file.readlines()
        current = int(lines[0]) + int(lines[1]) + int(lines[2])
        for i in range(1, len(lines) - 2):
            rolling_sum = int(lines[i]) + int(lines[i + 1]) + int(lines[i + 2])
            if rolling_sum > current:
                res += 1
            current = rolling_sum
    print(res)
