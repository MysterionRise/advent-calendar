...
start cave, you may not return to it, and once you reach the end cave,
...
def dfs(start, end, graph, visited, path, small_node):
    """
    Depth first search
    """
    ...
    if start == end:
        print(path)
        ALL_PATHS.add(tuple(path.copy()))
    else:
        sorted_nodes = graph[start]
        sorted_nodes.sort(reverse=True)
        for node in sorted_nodes:
            if node.islower() and node != small_node and visited[node] == 0:
                dfs(node, end, graph, visited, path, small_node)
            elif node == small_node and visited[node] <= 1:
                dfs(node, end, graph, visited, path, small_node)
            elif node != "start" and node.isupper():
                dfs(node, end, graph, visited, path, small_node)
...
def main():
    ...
    with open("passage_pathing.txt", encoding="utf-8") as file:
        ...
        for node in nodes:
            if node != "start" and node != "end" and node.islower():
                ...
                visited["start"] = 1
                dfs("start", "end", graph, visited, path, node)
        print(ALL_PATHS)
        print(len(ALL_PATHS))
