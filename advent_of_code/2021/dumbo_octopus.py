...
def main():
    ...
    """
    with open("dumbo_octopus.txt", encoding="utf-8") as file:
        lines = file.readlines()
        size_x = len(lines)
        size_y = len(lines[0].strip())
        octopuses = [[0 for _ in range(size_y)] for _ in range(size_x)]
        for i, row in enumerate(lines):
            for j, val in enumerate(row.strip()):
                octopuses[i][j] = int(val)
        for step in range(10000):
            flashed = [[False for _ in range(size_y)] for _ in range(size_x)]
            for i, row in enumerate(octopuses):
                for j, _ in enumerate(row):
                    octopuses[i][j] += 1
            for i, row in enumerate(octopuses):
                for j, val in enumerate(row):
                    if val > 9 and not flashed[i][j]:
            ...
            for i, row in enumerate(flashed):
                for j, val in enumerate(row):
                    if val:
                        octopuses[i][j] = 0
            all_flashed = True
            for _, row in enumerate(flashed):
