...
If any pair is nested inside four pairs, the leftmost such pair explodes. If
any regular number is 10 or greater, the leftmost such regular number
splits. Once no action in the above list applies, the snailfish number is
reduced.
...
[[3,[2,[1,[7,3]]]],[6,[5,[4,[3,2]]]]] becomes [[3,[2,[8,0]]],[9,[5,[4,[3,
2]]]]] (the pair [3,2] is unaffected because the pair [7,3] is further to
...
rounded up. For example, 10 becomes [5,5], 11 becomes [5,6], 12 becomes [6,
6], and so on.

Here is the process of finding the reduced result of [[[[4,3],4],4],[7,[[8,
4],9]]] + [1,1]:
...
For example, the magnitude of [9,1] is 3*9 + 2*1 = 29; the magnitude of [1,
9] is 3*1 + 2*9 = 21. Magnitude calculations are recursive: the magnitude of
...
[0,[5,8]],[[1,7],[9,6]]],[[4,[1,2]],[[1,4],2]]], which reduces to [[[[7,8],
[6,6]],[[6,0],[7,7]]],[[[7,8],[8,8]],[[7,9],[0,6]]]].
...
def split_of_number(regular_number):
    """
    split regular number
    """
    return math.floor(regular_number / 2), math.ceil(regular_number / 2)


def split(line):
    """
    split whole line, e.g. finding number > 10, and doing split to pairs
    """
    idx = 0
    not_splitted = True
        ...
        if val == "[":
            ...
            if val != ",":
                if not_splitted and line[idx + 1] != "," and line[idx + 1] != "]":
                    ...
                    brackets_stack.append(str(left))
                    brackets_stack.append(",")
                    brackets_stack.append(str(right))
                    brackets_stack.append("]")
                    not_splitted = False
                    idx += 1
                else:
                ...
                brackets_stack.append(val)
        idx += 1
    print(f"split   {convert_stack_to_string(brackets_stack)}")
    return not_splitted, convert_stack_to_string(brackets_stack)
    ...
    while idx < len(line):
        ...
        if val == "]":
            if len(stack) >= 2:
                right = stack.pop()
                left = stack.pop()
                stack.append(magnitude_of_pair(left, right))
        elif val == "[":
            idx += 1
            continue
        else:
...
def convert_stack_to_string(stack):
    """
    convert stack of numbers, brackets, commas to string
    """
    return "".join(stack)
...
def explode(line):
    """
    run 1 explode operation on line
    """
    stack = deque()
    brackets_stack = deque()
    open_brackets = 0
        ...
        if val == "[":
            ...
            if accum != "":
                if not discard_right:
                    stack.append(int(accum) + right)
                    brackets_stack.append(str(int(accum) + right))
                    discard_right = True
                else:
                    brackets_stack.append(accum)
                    stack.append(int(accum))
                accum = ""
            brackets_stack.append(val)
        elif val == "]":
            open_brackets -= 1
            ...
            if val != ",":
                ...
                if accum:
                    ...
                    accum = ""
                brackets_stack.append(",")
        if open_brackets == 5 and not_exploded:
            comma_idx = line.index(",", idx)
            bracket_idx = line.index("]", idx)
            left = int(line[idx + 1 : comma_idx])
            right = int(line[comma_idx + 1 : bracket_idx])
            brackets_stack.pop()
            discard_right = False
            if stack and stack[-1] is not None:
                ...
                missing = deque()
                while brackets_stack[-1] != str(prev):
                    missing.append(brackets_stack.pop())
                brackets_stack.pop()
                brackets_stack.append(str(prev + left))
                while missing:
                    brackets_stack.append(missing.pop())
            idx = bracket_idx
            stack.append(0)
            brackets_stack.append("0")
            not_exploded = False
        idx += 1
    print(f"explode {convert_stack_to_string(brackets_stack)}")
    return not_exploded, convert_stack_to_string(brackets_stack)


def sum_of_snailfish(left_line, right_line):
    """
    sum of 2 snailfish lines
    """
    numbers_sum = sum_of_lines(left_line, right_line)
    while True:
        not_exploded, numbers_sum = explode(numbers_sum)
        not_splitted = False
        if not_exploded:
            not_splitted, numbers_sum = split(numbers_sum)
        if not_splitted and not_exploded:
            break
    return numbers_sum
...
def main():
    ...
    """
    with open("snailfish.txt", encoding="utf-8") as file:
