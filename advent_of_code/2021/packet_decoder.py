...
def find_literal(line, idx):
    """
    finding literal in line of bits starting from idx
    """
    # literal there
    current_sum = 6
    group = "00000"
    while group[0] != "0":
        group = line[idx + 6 : idx + 11]
        idx += 5
        current_sum += 5
    if current_sum % 4 > 0:
        idx += 4 - (current_sum % 4)
...
def find_operator(line, idx):
    """
    finding operator in line of bits starting from idx
    """
