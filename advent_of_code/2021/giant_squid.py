...
def cross(num, board):
    """
    Cross out the numbers in the board that is equal to num
    :param num:
    :param board:
    :return:
    """
...
def sum_unchecked(board):
    """
    Sum the numbers in the board that are not marked (e.g. not equal to 0).
    Must remove 1 from each of the cell, due to encoding approach (where 0
    is marked) :param board: :return:
    """
    result_sum = 0
    for _, row in enumerate(board):
        for _, val in enumerate(row):
...
def test_sum_unchecked():
    """
    Test the sum_unchecked function
    :return:
    """
    board = [
        [0, 1, 2, 3, 4],
    ...
    assert sum_unchecked(board) == 184
...
def main():
    ...
    """
    with open("giant_squid.txt", encoding="utf-8") as file:
        lines = file.readlines()
        input_numbers = [int(ch) for ch in lines[0].split(",")]
        idx = 2
        boards = []
        current_board = [[0 for _ in range(5)] for _ in range(5)]
        while idx < len(lines):
            if lines[idx] == "\n":
                if current_board != [[0 for _ in range(5)] for _ in range(5)]:
                    boards.append(current_board)
                    current_board = [[0 for _ in range(5)] for _ in range(5)]
            else:
                line = lines[idx].split()
                for idx, val in enumerate(line):
                    current_board[(idx - 2) % 5][idx] = int(val) + 1
            idx += 1
        if current_board != [[0 for _ in range(5)] for _ in range(5)]:
            boards.append(current_board)
        ...
        for num in input_numbers:
            for board in boards:
                if not have_won[boards.index(board)]:
                    cross(num + 1, board)
                    if is_winner(board):
                        print(sum_unchecked(board) * num)
                        have_won[boards.index(board)] = True
